...
T do_mult_karatsuba(N *u, N *v, N *w, T m, T n)
{
    ...
    if (!is_null(k) && n > k)
    {
        ...
        // T t;
        UV0len = do_mult_karatsuba(U0, V0, W0, m-k, n-k); // mult U0*V0
        UV1len = do_mult_karatsuba(U1, V1, W2, k, k);     // mult U1*V1
        ...
        Clen = do_mult_karatsuba(C1, C2, C, C1len, C2len); // mult C = C1*C2
